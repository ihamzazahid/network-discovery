...
DEFAULT_OPT_TITLE   = 'Discovery Diagram'
...
def main():
    ...
    if (sys.argv[0] == 'list'):
        ...
        return
    if (sys.argv[0] == 'info'):
        print_mod_info(modules, sys.argv[1])
        return
    if (sys.argv[0] == 'help'):
        if (len(sys.argv) < 2):
            print_syntax()
            return
        print_mod_help(modules, sys.argv[1])
        return
    if (sys.argv[0] == 'syntax'):
        print_mod_syntax(modules, sys.argv[1])
        return

    mod = get_mod(modules, sys.argv[0])
    if (mod != None):
        exec_mod(mod, sys.argv[1:])
        return
...
def print_banner():
    print('Nets Discovery Version%s' % natlas.__version__)
    print('Hamza Zahid <hamza.zahid@nets-international.com>')
    print('Python %s\n' % sys.version.split(' ')[0])
